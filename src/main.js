// Talkchief AI Space — the page. Chrome, rail, chat, approvals, hotkeys and the boot hooks live
...
function boot() {
  ...
  tasks = (DEMO ? initTasks : initOfficeWork)({
    ...
    onLive: (h) => { const ver = document.querySelector('#topbar .brand .ver'); if (ver) ver.textContent = ''; document.title = h.name; brainNotes = h.notes; office.setBrainCount(h.notes); brain.setOwner(h.name); brain.setQuiet(true); applyRoster(h.agents); },

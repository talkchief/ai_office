...
export function initOfficeWork(ctx) {
  ...
  panel.innerHTML = `<button type="button" id="tpanelHandle" aria-label="Expand or collapse the work panel"></button><form class="space-command">
    ...
    <details class="space-options" id="spaceOptions"><summary><span class="opt-pill">Options <i aria-hidden="true">⌄</i></span><span class="opt-hint">assign · due date · more teams · documents</span></summary><div class="space-options-grid"><label>For<select id="spaceAssignee"></select></label><label>Due<input type="datetime-local" id="spaceDue"></label><label>Priority<select id="spacePriority"><option value="1">Normal</option><option value="2">High</option><option value="0">Low</option></select></label><label>Documents<input type="file" id="spaceFiles" multiple accept=".pdf,.docx,.txt,.md,.csv"></label><label>Project<select id="spaceProject"><option value="">None</option></select></label><div class="space-involve" id="spaceInvolve"></div></div></details>
  ...
  function taskActions(job) {
    ...
    if(job.realState==='awaiting_ceo'&&(job.pendingActions||[]).some(a=>a.name!=='complete_task'))return `<div class="space-owner-approval">${job.pendingActions.map(a=>`<div class="space-pending-action"><b>${esc(a.name==='complete_task'?'Close the task and file the result':'The team wants to run '+a.name)}</b>${a.name==='complete_task'?'':`<pre>${esc(JSON.stringify(a.args,null,2))}</pre>`}</div>`).join('')}<p>This acts outside the office. Approve to let the team run it once.</p><button data-action="approve">Approve</button></div><details data-detail-key="revision" open><summary>Reject with a note</summary><label>What should the team do instead?<textarea id="spaceRevision" rows="2"></textarea></label><div class="space-actions"><button class="secondary" data-action="reject">Reject</button></div></details>`;
    if(job.realState==='escalated')return `<details data-detail-key="revision" open><summary>The team needs your direction</summary>${job.error?`<p>${esc(job.error)}</p>`:''}<label>Your answer<textarea id="spaceRevision" rows="3"></textarea></label><div class="space-actions"><button data-action="answer">Send to the team</button><button class="space-text-action" data-action="cancel">Cancel task</button></div></details>`;
    if(job.state==='done')return `<details data-detail-key="revision"><summary>Ask for changes</summary><label>What should change? The lead reworks it and reviews it again, and you get a new version.<textarea id="spaceRevision" rows="3"></textarea></label><label class="space-check"><input type="checkbox" id="spaceRemember">Remember this for the team from now on</label><div class="space-actions"><button data-action="message" data-kind="correction">Send correction</button></div></details>`;
    ...
    if(['queued','planning','working','reviewing'].includes(job.state))return `<details data-detail-key="revision"><summary>Add a note for the team</summary><label>The team gets it at its next step.<textarea id="spaceRevision" rows="2"></textarea></label><div class="space-actions"><button data-action="message" data-kind="note">Send note</button><button class="space-text-action" data-action="cancel">Cancel task</button></div></details>`;

...
export function initOfficeWork(ctx) {
  ...
  for (const [id, label] of [['spaceModels', 'Models & keys'], ['spaceRoutines', 'Routines'], ['spaceOfficeSettings', 'Office settings'], ['spaceAudit', 'Audit log']]) if (!$(id)) { const b = document.createElement('button'); b.id = id; b.type = 'button'; b.innerHTML = `${label} <span>↗</span>`; $('spaceManageMenu').insertBefore(b, $('claudeConnect')); }

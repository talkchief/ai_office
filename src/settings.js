...
import { canShow } from './file-preview.js';
...
export function initSettings({ api, openTask, brain, syncBrain, onShow, onHide }) {
  ...
  async function showOffice() {
    try {
      ...
      content.innerHTML = `<form id="setOffice" data-dirty novalidate><div class="mg-controls">
        ...
        ${check(`<input type="checkbox" class="mg-switch" name="sandbox" ${s.sandbox !== false ? 'checked' : ''}>`, 'Sandbox for code', 'People you give “Sandbox (run code)” under Teams & people run Python and Node in a throwaway container of their task’s own, with no internet: to compute, simulate, test code and make files such as decks with charts. Installing a package waits for your approval while “Ask before outside actions” is on. The sandbox is deleted when the task is delivered. Off, and nobody runs code.')}
        <div class="mg-control"><span>Sandbox service</span><small id="spaceSandboxStatus">Checking whether sandboxes can run on this server…</small></div>
        ${num('sandboxCommandMinutes', 'A sandbox command may run for', 1, 15, 1, 'Agents may ask for a shorter limit, never a longer one.', 'min')}
        ${num('sandboxIdleMinutes', 'Remove an idle sandbox after', 5, 240, 1, 'For a task that is waiting (for you, or blocked). Its files stay with the task; the next command starts a fresh sandbox.', 'min')}
      ...
      api('/sandbox?fresh=1').then(sb => { const el = $('spaceSandboxStatus'); if (!el) return; el.innerHTML = sb.available ? `${mark('ok', 'Ready')} ${sb.image ? esc(sb.image) + ' · ' : ''}${sb.running?.length ? `${sb.running.length} running now` : 'none running now'}${sb.maxSandboxes ? ` · up to ${sb.maxSandboxes} on this server, ${sb.maxBusy} busy at once` : ''}` : `${mark(sb.enabled ? 'warn' : 'off', sb.enabled ? 'Not available' : 'Off')} ${esc(sb.reason || '')}`; }).catch(error => { const el = $('spaceSandboxStatus'); if (el) el.textContent = error.message; });
  ...
  async function agencyPicker(host, { mode, dept = '', full = false, teams = [], onDone }) {
    ...
    // Who is already working here, read fresh each time the picker opens: a persona is hired once per office.
    let hired = {}; if (mode === 'hire') { try { ({ hired } = await api('/agency/hired')); } catch {} }
    ...
    const list = () => {
      ...
      const hiredAs = p => { const h = mode === 'hire' && hired[p.id]; return h ? `<span class="agency-hired" title="Already working in this office: open ${esc(h.team)} to change their work, or remove them there to hire this persona again">Hired · ${esc(h.name)}, ${esc(h.team)}${h.lead ? ' (lead)' : ''}</span>` : ''; };
      $('agencyList').innerHTML = hits.map(p => `<article class="agency-item${hiredAs(p) ? ' hired' : ''}"${hiredAs(p) ? ' aria-disabled="true"' : ''}><span class="mg-avatar agency-avatar" aria-hidden="true">${esc(initials(p.name))}</span><div class="agency-body"><b>${esc(p.name)}</b><small>${esc(p.label)}${p.role && !p.description.startsWith(p.role.slice(0, 24)) ? ' · ' + esc(p.role) : ''}</small><p>${esc(p.description.slice(0, 180))}${p.description.length > 180 ? '…' : ''}</p></div><div class="agency-actions">${mode === 'hire' ? hiredAs(p) || `<button type="button" class="mg-btn mg-btn-sm mg-btn-primary" data-hire="${esc(p.id)}" ${full ? 'disabled' : ''}>Hire</button><button type="button" class="space-text-action" data-hire-lead="${esc(p.id)}">as the lead</button>` : `<button type="button" class="mg-btn mg-btn-sm mg-btn-primary" data-skill="${esc(p.id)}">Add method</button>`}</div></article>`).join('') || '<p class="agency-empty">No persona matches. Try a role, like “analyst” or “designer”.</p>';
  ...
  function briefProject(teams) {
    ...
    content.innerHTML = `<div class="mg-toolbar"><button type="button" class="mg-btn mg-goback" id="spaceBackProjects">← All projects</button></div>
      <form id="spaceProjectBrief" data-dirty novalidate><div class="mg-card"><div class="mg-card-head"><h3>New project</h3><span class="mg-count">the Program Manager plans it</span></div>
        ...
        ${field('Documents', '<input type="file" name="files" multiple accept=".pdf,.docx,.xlsx,.xls,.pptx,.txt,.md,.csv,.json">', 'Up to 10 files, 5 MB each. PDF, Word, Excel (XLSX, XLS), PowerPoint, text, Markdown, CSV or JSON. They are filed in the project’s Brain folder and given to every task.')}
    ...
    $('spaceProjectBrief').onsubmit = async event => {
      ...
      if (picked.some(f => f.size > 5 * 1024 * 1024)) return feedback('A file is larger than 5 MB.', true);
  ...
  // What the office can draw for itself; anything else is only worth saving.
  const canPreview = row => canShow(row.name) && Boolean(row.name) && !/\.(xlsx?|docx?|pptx?)$/i.test(String(row.name || ''));
  const previewLink = (url, name, bytes) => `<button type="button" class="mg-btn mg-btn-sm" data-preview-url="${esc(url)}" data-preview-name="${esc(name)}" data-preview-bytes="${bytes || 0}" title="Read it here">Preview</button>`;
  ...
  const projectWork = (detail, teams, roster) => {
    ...
    const task = t => { const c = TASK_CHIP(t.state), steps = t.subtasks || [];
      // Who is on it: the specialists the work was given to, and the lead when nothing has been handed out yet.
      const people = [...new Set(steps.map(s => nameOf(s.agent)).filter(Boolean))];
      ...
      + (steps.length ? `<div class="pw-steps" data-steps="${esc(t.id)}" hidden>${steps.map(s => { const sc = STEP_CHIP(s.state);
        return `<div class="pw-row pw-step" data-open-task="${esc(t.id)}" role="button" tabindex="0" title="Open the task this belongs to"><div class="pw-name"><span class="pw-tree pw-tree-2" aria-hidden="true"></span>${pwRing(sc[0])}<span class="pw-tt">${esc(String(s.title).slice(0, 120))}</span></div><div class="pw-teamcol"></div><div class="pw-who">${esc(nameOf(s.agent) || '')}</div><div class="pw-progress"></div><div class="pw-status">${pwChip(sc)}</div><div class="pw-when"></div></div>`; }).join('')}</div>` : ''); };
  ...
  const fileLink = a => canShow(a.name)
    ? `<span class="mg-fileset"><button type="button" class="mg-filelink" data-preview-url="${esc(a.url)}" data-preview-name="${esc(a.name)}" data-preview-bytes="${a.bytes || 0}" title="Read ${esc(a.name)} here">${fileIcon(a.name)}<span>${esc(String(a.name).split('/').pop())}</span></button><a class="mg-filedl" href="${esc(a.url)}" download title="Download ${esc(a.name)}" aria-label="Download ${esc(a.name)}">↓</a></span>`
    : `<a class="mg-filelink" href="${esc(a.url)}" download title="Download ${esc(a.name)}">${fileIcon(a.name)}<span>${esc(String(a.name).split('/').pop())}</span></a>`;
  ...
  async function editProject(id, teams) {
    try {
      ...
      content.innerHTML = `<div class="mg-toolbar"><button type="button" class="mg-btn mg-goback" id="spaceBackProjects">← All projects</button><span class="mg-spacer"></span>${p.id ? `<label>State <select id="spaceProjectStatus">${['active', 'paused', 'done', 'archived'].map(st => `<option value="${st}" ${st === p.status ? 'selected' : ''}>${st}</option>`).join('')}</select></label><button type="button" class="mg-btn mg-btn-sm mg-btn-danger" id="spaceDeleteProject">Delete project</button>` : ''}</div>
        ...
        ${p.id ? `<div class="mg-card" style="margin-top:16px"><div class="mg-card-head"><h3>Files</h3><span class="mg-count">${detail.files.length}</span></div><p>Uploads land in the project's Brain folder, where every task of this project can read them.</p><div class="mg-upload"><input type="file" id="spaceProjectFiles" multiple accept=".pdf,.docx,.xlsx,.xls,.pptx,.txt,.md,.csv,.json"><button type="button" class="mg-btn mg-btn-sm" id="spaceProjectUpload">Upload</button><small>PDF, Word, Excel (XLSX, XLS), PowerPoint, text, Markdown, CSV or JSON · up to 5 MB each.</small></div>
          ${detail.files.length ? `<div class="mg-ledger-wrap" style="margin:0"><table class="mg-ledger"><tbody>${detail.files.map(f => `<tr><td>${fileIcon(f.id)} <span class="mg-name">${esc(f.title || f.id.split('/').pop())}</span><span class="mg-sub">${esc(f.id)}</span></td><td class="k r">${f.updatedAt ? esc(when(f.updatedAt)) : ''}</td><td class="r">${canShow(f.id) ? `<button type="button" class="mg-btn mg-btn-sm" data-preview-url="/api/knowledge/file?id=${encodeURIComponent(f.id)}" data-preview-name="${esc(f.id)}" data-preview-bytes="${f.bytes || 0}" title="Read it here">Preview</button>` : ''}<a class="mg-btn mg-btn-sm" href="/api/knowledge/file?id=${encodeURIComponent(f.id)}" download>Download</a></td></tr>`).join('')}</tbody></table></div>` : ''}</div>
        ` : ''}
        </div>
  ...
  async function showBrain() {
    try {
      ...
      $('spaceUploadForm').onsubmit = async event => {
        ...
        try {
          const done = [];
          for (const file of files) { if (file.size > 5 * 1024 * 1024) throw new Error(`${file.name} is larger than 5 MB.`); toast(`Reading ${file.name}…`, { kind: 'info', ms: 2500 }); const r = await api('/knowledge/upload', 'POST', { folder: form.elements.folder.value, name: file.name, data: await readFile(file) }); done.push(`${r.id}${r.replaced ? ' (replaced)' : ''}`); }
    ...
    try {
      ...
      if (adminTab === 'limits') {
        body.innerHTML = `<form id="platformForm" data-dirty novalidate><div class="mg-card"><div class="mg-card-head"><h3>Limits</h3><span class="mg-count">apply to the next change in every office</span></div><div class="mg-grid">
          ...
          <div class="mg-card"><div class="mg-card-head"><h3>Sandboxes</h3><span class="mg-count">${cfg.sandbox?.allowed !== false ? 'offices may run code' : 'off for every office'}</span></div>
          ${check(`<input type="checkbox" class="mg-switch" name="sandboxAllowed" ${cfg.sandbox?.allowed !== false ? 'checked' : ''}>`, 'Let offices run code in sandboxes', 'On by default. Each task gets a throwaway container with no internet, run by the sandbox service on this server, and only the teams and people an office gives “Sandbox (run code)” can use it (nobody has it until then). Off, and no office sees the tool.')}</div>
          <div class="mg-card"><div class="mg-card-head"><h3>Sign-in protection</h3><span class="mg-count">${cfg.turnstile.enabled ? 'Cloudflare Turnstile is on' : cfg.turnstile.siteKey || cfg.turnstile.hasSecretKey ? 'both keys are needed' : 'off'}</span></div><div class="mg-grid">
          ${field('Turnstile site key', `<input name="turnstileSiteKey" value="${esc(cfg.turnstile.siteKey || '')}" placeholder="0x4AAAAAAA…" autocomplete="off">`, 'From Cloudflare → Turnstile → your widget. This half is public: it goes into the sign-in page.')}
          ${field('Turnstile secret key', `<input name="turnstileSecretKey" type="password" autocomplete="off" placeholder="${cfg.turnstile.hasSecretKey ? 'Stored · paste to replace' : 'Paste the secret key'}">${cfg.turnstile.hasSecretKey || cfg.turnstile.siteKey ? '<label class="mg-check" style="margin:6px 0 0;font-size:11px"><input type="checkbox" name="clearTurnstile"> Turn Turnstile off and forget both keys</label>' : ''}`, 'Kept on this server only, never shown again and never sent to the page. With both keys saved, signing in, registering and accepting an invitation must pass the challenge.')}</div></div>

...
const SECTIONS = [['office', 'Office'], ['teams', 'Teams & people'], ['models', 'Models & keys'], ['tools', 'Tools & connectors'], ['vault', 'Vault'], ['skills', 'Skills'], ['projects', 'Projects'], ['artifacts', 'Office Artifacts'], ['routines', 'Routines'], ['reports', 'Reports & KPIs'], ['brain', 'Brain'], ['audit', 'Audit log']];
...
export function initSettings({ api, openTask, brain, syncBrain, onShow, onHide }) {
  ...
  let vaultEditing = null;
  async function showVault() {
    try {
      const [data, config] = await Promise.all([api('/vault'), api('/office')]);
      if (section !== 'vault') return;
      const teams = config.teams || [], kindLabel = { api: 'Outside service (API)', database: 'Database connection', ssh: 'SSH target' };
      ...
      content.innerHTML = `<p>A secret store like a code host’s. Add a key, a database password or an SSH credential once; agents use it through the office (an outside service through <code>api_get</code> and <code>api_request</code>, a database through <code>db_query</code> and <code>db_write</code>, an SSH target through <code>ssh_run</code>) and never see, type or ask for the value. Anything that changes data outside the office still pauses for your approval.</p>
        <div class="space-actions"><button type="button" id="vaultNew">+ New entry</button><span class="space-count">${data.entries.length} entr${data.entries.length === 1 ? 'y' : 'ies'}</span></div>
        ${data.entries.map(x => `<div class="audit-item vault-item"><b>${esc(x.name)}</b> <small>${esc(x.id)} · ${esc(kindLabel[x.kind] || x.kind)}${x.kind === 'api' ? ' · ' + esc(x.baseURL) : ' · ' + esc(x.host || '') + (x.port ? ':' + x.port : '')}${x.username ? ' · ' + esc(x.username) : ''}${x.kind === 'database' ? ' · ' + (x.readOnly === false ? 'writable' : 'read-only') : ''}${x.kind === 'ssh' ? ' · ' + (x.allow?.length ? x.allow.length + ' allowed command prefix' + (x.allow.length === 1 ? '' : 'es') : 'any command') : ''}</small><div><small>${x.hasSecret ? 'Secret stored' : 'No secret yet'} · ${x.teams?.length ? 'Teams: ' + x.teams.map(t => esc(teams.find(y => y.id === t)?.name || t)).join(', ') : 'Every team'}${x.notes ? ' · ' + esc(x.notes) : ''}</small> <button type="button" class="space-link" data-vault-edit="${esc(x.id)}">Edit</button> <button type="button" class="space-link" data-vault-remove="${esc(x.id)}">Remove</button></div></div>`).join('') || '<div class="space-empty"><h3>The Vault is empty.</h3><p>Add the first entry: for example here.now, so the Website Publisher can publish a site.</p></div>'}
        ${e ? `<form id="vaultForm" class="space-form" data-dirty><h3>${esc(e.updatedAt ? 'Edit ' + e.name : 'New entry')}</h3>
          <div class="space-grid"><label>Id<input name="id" value="${esc(e.id)}" ${e.updatedAt ? 'readonly' : ''} placeholder="here.now" required></label><label>Kind<select name="kind">${['api', 'database', 'ssh'].map(k => opt(k, e.kind || 'api', kindLabel[k])).join('')}</select></label><label>Name<input name="name" value="${esc(e.name || '')}" placeholder="here.now (website hosting)"></label></div>
          <div class="space-grid" data-kind="api"><label>Base URL<input name="baseURL" value="${esc(e.baseURL || '')}" placeholder="https://here.now/api/v1"></label><label>Header<input name="authHeader" value="${esc(e.authHeader || 'Authorization')}"></label><label>Prefix<input name="authPrefix" value="${esc(e.authPrefix ?? 'Bearer ')}"></label></div>
          <div class="space-grid" data-kind="database ssh" hidden><label>Host<input name="host" value="${esc(e.host || '')}"></label><label>Port<input name="port" type="number" value="${esc(e.port || '')}"></label><label>User<input name="username" value="${esc(e.username || '')}"></label><label data-kind="database">Database<input name="database" value="${esc(e.database || '')}"></label><label data-kind="database">Engine<select name="engine">${[['postgres', 'Postgres'], ['mysql', 'MySQL / MariaDB']].map(([k, label]) => opt(k, /mysql|maria/i.test(e.engine || '') ? 'mysql' : 'postgres', label)).join('')}</select></label></div>
          <label class="space-check" data-kind="database"><input type="checkbox" name="readOnly" ${e.readOnly === false ? '' : 'checked'}> Read-only: agents may only read from it. Untick to let <code>db_write</code> change data, each statement approved by you first.</label>
          <label data-kind="ssh">Allowed commands (one prefix per line, such as <code>git pull</code> or <code>systemctl status</code>; empty means any command; you approve every command either way)<textarea name="allow" rows="3">${esc((e.allow || []).join('\n'))}</textarea></label>
          <label data-kind="ssh">Host key fingerprint (optional: <code>SHA256:…</code> from <code>ssh-keygen -lf</code>; when set, a host whose key differs is refused)<input name="fingerprint" value="${esc(e.fingerprint || '')}" placeholder="SHA256:…"></label>
          <label>Secret (${e.hasSecret ? 'stored; leave blank to keep it' : 'the key, password or private key'})<textarea name="secret" rows="2" autocomplete="off" placeholder="${e.hasSecret ? '••••••••' : ''}"></textarea></label>
          <fieldset class="space-fieldset"><legend>Teams that may use it (none ticked = every team)</legend>${teams.map(t => `<label class="space-check"><input type="checkbox" name="teams" value="${esc(t.id)}" ${e.teams?.includes(t.id) ? 'checked' : ''}> ${esc(t.name)}</label>`).join('')}</fieldset>
          <label>Notes for the agents<input name="notes" value="${esc(e.notes || '')}" placeholder="Publishes static sites; the flow is create → upload → finalize"></label>
          <div class="space-settings-save"><span>The secret is stored on the server and never shown again.</span>${e.hasSecret ? '<button type="button" id="vaultClear">Remove secret</button>' : ''}<button type="button" id="vaultCancel">Cancel</button><button type="submit">Save entry</button></div></form>` : ''}`;
      $('vaultNew').onclick = () => { vaultEditing = 'new'; showVault(); };
      content.querySelectorAll('[data-vault-edit]').forEach(b => b.onclick = () => { vaultEditing = b.dataset.vaultEdit; showVault(); });
      content.querySelectorAll('[data-vault-remove]').forEach(b => b.onclick = async () => { if (!confirm(`Remove “${b.dataset.vaultRemove}” from the Vault? Agents lose access at once.`)) return; try { await api(`/vault/${encodeURIComponent(b.dataset.vaultRemove)}`, 'DELETE'); vaultEditing = null; feedback('Removed from the Vault.'); showVault(); } catch (error) { feedback(error.message, true); } });
      ...
      if (form) {
        ...
        $('vaultCancel').onclick = () => { vaultEditing = null; showVault(); };
        if ($('vaultClear')) $('vaultClear').onclick = async () => { try { await api(`/vault/${encodeURIComponent(e.id)}`, 'PUT', { clearSecret: true }); feedback('Secret removed.'); showVault(); } catch (error) { feedback(error.message, true); } };
        form.onsubmit = async event => {
          ...
          try { await api(`/vault/${encodeURIComponent(id === 'new' ? f.id.value : id)}`, 'PUT', payload); vaultEditing = null; feedback(`Saved. Agents can use “${id}” from the next task.`); showVault(); } catch (error) { feedback(error.message, true); }
        };
      }
    } catch (error) { feedback(error.message, true); }
  }

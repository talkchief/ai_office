    ...
    try {
      ...
      if (adminTab === 'limits') {
        body.innerHTML = `<form id="platformForm" data-dirty novalidate><div class="mg-card"><div class="mg-card-head"><h3>Limits</h3><span class="mg-count">apply to the next change in every office</span></div><div class="mg-grid">
          ...
          <div class="mg-card"><div class="mg-card-head"><h3>Sandboxes</h3><span class="mg-count">${cfg.sandbox?.allowed ? 'offices may run code' : 'off for every office'}</span></div>
          ${check(`<input type="checkbox" class="mg-switch" name="sandboxAllowed" ${cfg.sandbox?.allowed ? 'checked' : ''}>`, 'Let offices run code in sandboxes', 'Each task gets a throwaway container with no internet, run by the sandbox service on this server; each office then switches it on and gives it to the teams that need it. Off, and no office sees the tool.')}</div>

...
export function initSettings({ api, openTask, brain, syncBrain, onShow, onHide }) {
  ...
  async function showVault() {
    try {
      ...
      content.innerHTML = `<p>A secret store like a code host’s. Add a key, a database password or an SSH credential once; agents use it through the office (an outside service through <code>api_get</code> and <code>api_request</code>, a database or SSH target through its connector) and never see, type or ask for the value. Anything that changes data outside the office still pauses for your approval.</p>
        ...
        ${data.entries.map(x => `<div class="audit-item vault-item"><b>${esc(x.name)}</b> <small>${esc(x.id)} · ${esc(kindLabel[x.kind] || x.kind)}${x.kind === 'api' ? ' · ' + esc(x.baseURL) : ' · ' + esc(x.host || '') + (x.port ? ':' + x.port : '')}${x.username ? ' · ' + esc(x.username) : ''}</small><div><small>${x.hasSecret ? 'Secret stored' : 'No secret yet'} · ${x.teams?.length ? 'Teams: ' + x.teams.map(t => esc(teams.find(y => y.id === t)?.name || t)).join(', ') : 'Every team'}${x.notes ? ' · ' + esc(x.notes) : ''}</small> <button type="button" class="space-link" data-vault-edit="${esc(x.id)}">Edit</button> <button type="button" class="space-link" data-vault-remove="${esc(x.id)}">Remove</button></div></div>`).join('') || '<div class="space-empty"><h3>The Vault is empty.</h3><p>Add the first entry: for example here.now, so the Website Publisher can publish a site.</p></div>'}

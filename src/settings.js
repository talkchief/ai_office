...
const SECTIONS = [['office', 'Office'], ['teams', 'Teams & people'], ['models', 'Models & keys'], ['tools', 'Tools & connectors'], ['skills', 'Skills'], ['routines', 'Routines'], ['reports', 'Reports & KPIs'], ['brain', 'Brain'], ['audit', 'Audit log']];
...
export function initSettings({ api, openTask, brain, syncBrain, onShow, onHide }) {
  ...
  let section = null, dirty = false, config = null, tools = [], providers = { models: [] }, draft = null, team = null, teamSection = 'overview', reportDays = 7, pendingNote = null, toolPoll = null;
  const feedback = (text, error = false) => { const m = $('settingsMessage'); m.textContent = text; m.classList.toggle('error', error); };
  content.addEventListener('input', event => { if (event.target.closest('form[data-dirty]')) dirty = true; });
  ...
  const RENDER = { office: showOffice, teams: showTeams, models: showModels, tools: showTools, skills: showSkills, routines: showRoutines, reports: showReports, brain: showBrain, audit: showAudit };
  ...
  function stopPoll() { if (toolPoll) { clearInterval(toolPoll); toolPoll = null; } }
  ...
  function renderTeam() {
    ...
    content.innerHTML = `<div class="space-settings-layout"><aside class="space-settings-nav"><span class="space-eyebrow">TEAMS</span><nav class="space-tabs">${draft.teams.map(x => `<button type="button" data-team="${x.id}" class="${x.id === t.id ? 'selected' : ''}">${esc(x.name)}</button>`).join('')}</nav><button type="button" class="secondary" id="spaceAddTeam">+ Add team</button></aside><div class="space-settings-body"><div class="space-settings-heading"><div><span class="space-eyebrow">TEAM</span><h3>${esc(t.name)}</h3><p>${agents.length} people · ${esc(agents.find(a => a.id === t.lead)?.name || 'no lead')} leads</p></div><button type="button" class="secondary" id="spaceRemoveTeam">Remove team</button></div>
      ...
      <label>Team purpose<textarea name="purpose" rows="2" placeholder="What does this team own, and what does success look like?">${esc(t.purpose)}</textarea></label><label>Working instructions<textarea name="instructions" rows="5" placeholder="Process, tone, source requirements and boundaries for this team.">${esc(t.instructions)}</textarea></label></section>
  ...
  async function showTools(editing = null) {
    try { [tools, config] = await Promise.all([api('/tools'), api('/office')]); renderTools(editing); } catch (error) { feedback(error.message, true); }
  }
  ...
  async function agencyPicker(host, { mode, dept = '', full = false, teams = [], onDone }) {
    host.hidden = !host.hidden; if (host.hidden) return;
    ...
    host.innerHTML = `<div class="agency-picker"><p>${mode === 'hire' ? 'Pick a ready-made specialist. They join this team with their role, standing instructions and their full method as a skill.' : 'Pick a method. It becomes a skill you can give to teams and people.'} <small>From the open-source Agency catalogue (MIT).</small></p>
      <div class="space-actions"><input type="search" id="agencyQ" placeholder="Search ${agencyIndex.personas.length} personas"><select id="agencyDivision"><option value="">All divisions</option>${divisions.map(([id, label]) => `<option value="${esc(id)}">${esc(label)}</option>`).join('')}</select></div>
      ${mode === 'skill' ? `<div class="space-actions"><label>Give it to<select id="agencyTeam"><option value="">Nobody yet</option>${teams.map(t => `<option value="${esc(t.id)}">${esc(t.name)}</option>`).join('')}</select></label></div>` : full ? '<p class="error">This team is full (a lead and six specialists). Remove someone first, or use "Hire as the lead" to give the lead a new role.</p>' : ''}
    ...
    const list = () => {
      ...
      $('agencyList').innerHTML = hits.map(p => `<article class="agency-item"><div><b>${esc(p.emoji || '')} ${esc(p.name)}</b><small>${esc(p.label)} · ${esc(p.role)}</small><p>${esc(p.description.slice(0, 220))}</p></div><div class="space-actions">${mode === 'hire' ? `<button type="button" data-hire="${esc(p.id)}" ${full ? 'disabled' : ''}>Hire</button><button type="button" class="secondary" data-hire-lead="${esc(p.id)}">Hire as the lead</button>` : `<button type="button" data-skill="${esc(p.id)}">Add method</button>`}</div></article>`).join('') || '<p>No persona matches.</p>';
      const act = (attr, fn) => $('agencyList').querySelectorAll(`[${attr}]`).forEach(b => b.onclick = async () => { b.disabled = true; try { const r = await fn(b.getAttribute(attr)); host.hidden = true; await onDone(r); } catch (error) { feedback(error.message, true); b.disabled = false; } });
  ...
  route();
  return { open, close, isOpen: () => !page.hidden, openNote: id => { pendingNote = id; open('brain'); if (section === 'brain') showBrain(); } };
}

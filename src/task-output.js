...
export function renderTaskWorkspace(job, tab, actions = '') {
  ...
  const artifacts = () => {
    ...
    return `<div class="space-artifacts"><p class="space-artifacts-intro">${files.length} file${files.length === 1 ? '' : 's'} in this task’s workspace. Documents the team exported come first.</p><ul class="space-artifact-list">${files.map(f => `<li class="space-artifact${exported(f.name) ? ' pdf' : ''}">${fileIcon(f.name)}<span class="space-artifact-body"><b>${esc(f.name)}</b><small>${size(f.bytes)} · ${esc(when(f.modifiedAt))}</small></span><a class="space-artifact-download" href="/api/tasks/${job.id}/file?path=${encodeURIComponent(f.name)}" download>Download</a></li>`).join('')}</ul></div>`;
  };

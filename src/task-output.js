...
export function renderTaskWorkspace(job, tab, actions = '') {
  ...
  const head = `<div class="tv-head"><div class="tv-top"><span class="mg-eyebrow">Task · ${esc(job.team.name)} · ${esc(lead)}</span>${stateMark(job)}${job.lane === 'quick' ? '<span class="tv-chip" title="Quick work: the lead delivered and reviewed it alone">Quick lane</span>' : ''}${job.projectName ? `<span class="tv-chip">${esc(job.projectName)}</span>` : ''}${job.archivedAt ? '<span class="tv-chip" title="Off the board; everything is kept">Archived</span>' : ''}</div><h1 class="tv-title">${esc(job.title)}</h1>
    <div class="tv-facts">${job.createdAt ? `<span>created <b>${esc(short(job.createdAt))}</b></span>` : ''}${dur ? `<span>took <b>${esc(dur)}</b></span>` : ''}${job.dueAt ? `<span>due <b>${esc(short(job.dueAt))}</b></span>` : ''}${job.state === 'done' && (job.doneAt || job.review?.at) ? `<span>approved <b>${esc(short(job.doneAt || job.review.at))}</b> · filed in the Brain</span>` : ''}${total ? `<span><b>${submitted}</b> of <b>${total}</b> assignments submitted</span>` : ''}${job.sandbox?.runs || job.sandbox?.installs ? `<span title="Commands run in the task's sandbox and the time they took">sandbox <b>${job.sandbox.runs || 0}</b> command${job.sandbox.runs === 1 ? '' : 's'}${job.sandbox.seconds ? ` · <b>${esc(String(job.sandbox.seconds))}</b> s` : ''}${job.sandbox.installs ? ` · <b>${job.sandbox.installs}</b> install${job.sandbox.installs === 1 ? '' : 's'}` : ''}</span>` : ''}</div></div>`;
  ...
  const decision = () => {
    ...
    else if (job.archivedAt) { kind = 'off'; glyph = '▤'; title = `Archived on ${short(job.archivedAt)}.`; text = job.state === 'done' ? 'Off the board, everything kept: the result is still filed in the Brain, with its conversation and files. Restore it to ask for changes.' : 'Off the board. Its conversation and files are kept.'; }

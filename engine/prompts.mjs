...
2. Write the deliverable to a file under /work/ (Markdown unless the CEO asked for another format). For a PDF, write the Markdown first and call export_pdf; for slides, write the deck as Markdown (one ## heading per slide, 3 to 6 short bullets each) and call export_pptx. Export once: write the Markdown to the length the CEO asked for and, when the CEO asked for a page count, pass it as pages to export_pdf (the office fits the print to it); do not re-export to tune page counts, and never read the export back (it is binary; the Markdown is the source). A short answer (a fact, a date, a file name) can be the text itself.

...
## The sandbox: code in a throwaway container per task

People you give **Sandbox (run code)** (Teams & people → Tools & skills, per team or per person) can run code: Python 3.12 with pandas, numpy, scipy, matplotlib, simpy, python-pptx, python-docx, openpyxl, reportlab and pytest; Node 22 with npm; git and build tools. They use it to compute and check numbers, run simulations, analyse data, test code they wrote, and make files the export tools cannot (a deck with charts, generated data). Each task gets its own Linux container, started on the first command and **removed when the task is delivered**, cancelled or deleted, after 20 idle minutes, and at start-up for tasks that closed meanwhile.

- **Nothing reaches the office's disk but plain files.** The container never sees the task's folder: before each command the office copies `/work/` in, and after it the new and changed files come back, plain files only (a link, a pipe or a device made inside is refused and named in the answer; `node_modules`, `__pycache__` and `.git` stay inside).
- **No internet.** A package the image lacks comes through `sandbox_install` (pip or npm by name and version only): a short-lived helper container that has the network but never the task's files, runs no install scripts (pip takes wheels only, npm `--ignore-scripts`), and waits for your approval while "Ask before outside actions" is on. Running code needs no approval.
- **Limits.** 1 GB of memory, 1 CPU, 256 processes, a read-only root, every capability dropped, a non-root user; a command stops after 5 minutes unless the agent asks for up to 15 (Settings → Office); `/work` inside is capped at 2 GB; at most 8 sandboxes and 3 busy commands on the server at once.
- **Who runs the containers.** A broker service under its own unprivileged user (`ao-sandbox`, rootless Podman) on a local socket, with one fixed container profile: the office cannot ask it for an image, a mount, a user, a network or a privilege. Set it up once as root with `deploy/sandbox/setup.sh` (user, subordinate ids, CPU and memory delegation, the image, the `ao-sandbox-broker` service), restart the office service, then run `node scripts/sandbox-smoke.mjs`. `deploy/sandbox/setup.sh image` rebuilds the image after a change to `sandbox/Containerfile`.
- **Switches.** Nobody has the tool until you give it to a team or a person; that is the switch that starts off. Settings → Office → Sandbox for code turns it off for the whole office (on by default; it also shows the command and idle limits and whether the service answers), and in hosted mode Platform → Settings → Sandboxes turns it off for every office (on by default). The task page shows the commands run and their time; each command, install and removal is a line on the timeline.

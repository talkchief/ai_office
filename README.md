...
- **Switches.** Settings → Office → Sandbox for code (on by default, with the command and idle limits and whether the service answers). In hosted mode the platform administrator first allows it under Platform → Settings → Sandboxes (off by default). The task page shows the commands run and their time; each command, install and removal is a line on the timeline.

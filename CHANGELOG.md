...
- **The Agency.** 2,242 open-source specialist personas (MIT) in 24 divisions ship with the office, each with a real job title, a one-line description and search tags; search ranks by job, so "developer" lists developers first. Hire one onto a team from Settings (they carry the same initials avatar as everyone on a team), or take just its method as a skill.

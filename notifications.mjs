...
export class Notifications {
  ...
  row(r) { return r && { id: r.id, at: r.at, kind: r.kind, severity: r.severity, title: r.title, body: r.body || '', jobId: r.job_id, agent: r.agent, dept: r.dept, action: r.action ? JSON.parse(r.action) : null, readAt: r.read_at, ackedAt: r.acked_at }; }
  ...
  notify({ kind, title, body = '', jobId = null, agent = null, dept = null, action = null, severity, dedupe = null }) {
    ...
    this.db.prepare('INSERT INTO office_notifications(id, at, kind, severity, title, body, job_id, agent, dept, action, dedupe) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
      .run(item.id, item.at, kind, level, String(title).slice(0, 300), String(body).slice(0, 4000), jobId, agent, dept, action ? JSON.stringify(action) : null, dedupe);
    const saved = this.get(item.id); this.bus?.publish('notification.new', saved); return saved;
  }
  list({ limit = 100, unread = false, open = false } = {}) {
    const where = [unread ? 'read_at IS NULL' : '', open ? 'acked_at IS NULL' : ''].filter(Boolean).join(' AND ');
    return this.db.prepare(`SELECT * FROM office_notifications ${where ? 'WHERE ' + where : ''} ORDER BY at DESC, rowid DESC LIMIT ?`).all(Math.min(500, limit)).map(r => this.row(r));
  }
  counts() {
    const r = this.db.prepare("SELECT SUM(read_at IS NULL) AS unread, SUM(severity = 'action' AND acked_at IS NULL) AS needsYou FROM office_notifications").get();
  ...
  readAll() { const ids = this.db.prepare('SELECT id FROM office_notifications WHERE read_at IS NULL').all().map(r => r.id); this.db.prepare('UPDATE office_notifications SET read_at = ? WHERE read_at IS NULL').run(Date.now()); this.bus?.publish('notification.read', { ids }); return this.counts(); }

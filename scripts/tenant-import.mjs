...
//
//   node scripts/tenant-import.mjs --tenant <tenantId> [--data ./data] [--brain ./brain] [--tenants ./tenants]
//
...
const SKIP = new Set(['workspaces', 'knowledge-index', 'providers.json', 'accounts.sqlite', 'platform']);
...
export async function importTenant({ tenantId, data, brain, tenants }) {
  ...
  if (fs.existsSync(path.join(data, 'workflows.sqlite-wal'))) throw new Error('The office looks like it is running (workflows.sqlite-wal exists). Stop it first.');
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    const target = await importTenant({ tenantId: flag('--tenant'), data: path.resolve(flag('--data') || process.env.AO_DATA || path.join(ROOT, 'data')), brain: path.resolve(flag('--brain') || cfg.brainPath), tenants: path.resolve(flag('--tenants') || process.env.AO_TENANTS_DIR || path.join(ROOT, 'tenants')) });
